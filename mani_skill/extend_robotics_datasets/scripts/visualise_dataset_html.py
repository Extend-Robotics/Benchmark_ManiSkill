...
"""
Few modifications made to make it work properly
"""
...
```bash
local$ python -m extend_robotics_datasets.scripts.visualize_dataset_html \
...
```bash
distant$ m extend_robotics_datasets.scripts.visualize_dataset_html \
    --repo-id lerobot/pusht
...
```bash
python m extend_robotics_datasets.scripts.visualize_dataset_html \
    --repo-id lerobot/pusht \
    --episodes 7 3 5 1 4
```
"""
...
def run_server(
    ...
    def hommepage(dataset=dataset):
        if dataset:
            if "/" in dataset.repo_id:
                dataset_namespace, dataset_name = dataset.repo_id.split("/")
            else:
                dataset_namespace = "lerobot"
        ...
        return render_template(
            "visualise_dataset_homepage.html",
    ...
    def show_episode(
        ...
        return render_template(
            "visualise_dataset_template.html",
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Episode indices to visualise (e.g. `0 1 5 6` to load episodes of index 0, 1, 5 and 6). By default loads all episodes.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=None,
        help="Directory path to write html files and kickoff a web server. By default write them to 'outputs/visualise_dataset/REPO_ID'.",
    )
    parser.add_argument(
        "--serve",
        type=int,
    ...
    visualise_dataset_html(dataset, **vars(args))
